...
diff_signal = np.diff(traj_log_y)
for i in range(len(diff_signal)):
    if abs(diff_signal[i]) > 2e-4:
        index_start = i
        break

traj_log_x_index = traj_log_x[index_start:index_start+151]
traj_log_y_index = traj_log_y[index_start:index_start+151]
traj_log_z_index = traj_log_z[index_start:index_start+151]
time_log_index = time_log[index_start:index_start+151]
